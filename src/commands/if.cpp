...
void If::execute(Resolver & resolver, std::map<std::string, std::pair<std::string, int>> * globalDataPool, std::vector<std::unique_ptr<Command>> & cmds, int * cmdPtr)
{
    ...
    if(intVal < 1){
        *cmdPtr = resolver.findLabel("If", "EndIf", 1, false, cmds, cmdPtr);
    }
}
